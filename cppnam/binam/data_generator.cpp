    ...
    const DataGenerator::ProgressCallback &progress)
{
	...
	for (size_t i = 0; i < n_samples; i++) {
		...
		for (size_t j = 0; j < n_ones; j++) {
			...
			if (balance) {
				// If the number of bits that have minimum usage is smaller than
				// the number of remaining bits, allow a one-off error (slack)
				...
				uint8_t slack = 0;
				{
					...
					for (size_t k = 0; k < idx; k++) {
						idcs_with_min_usage +=
						    selected[k] && (usage[k] - min_usage == 0) ? 1 : 0;
					}
